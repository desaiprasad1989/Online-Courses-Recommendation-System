...
@st.cache_resource
def load_assets():
    item_user_matrix = joblib.load("models/cf_item_user_matrix.joblib")

    
    if not isinstance(item_user_matrix, csr_matrix):
    ...
    return (
        item_user_matrix, tfidf_matrix, item_ids, hybrid_config, courses_df,
...
def get_cf_scores(course_id, top_k=20):
    ...
    item_vec = item_user_matrix[idx]

    sims = cosine_similarity(
        item_vec,
        item_user_matrix,
        dense_output=False   # 🔥 CRITICAL
    )
